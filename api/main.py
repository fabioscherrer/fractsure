...
def decode_detections(
    ...
    if rows.shape[1] == 6:
        ...
        with_obj = decode_xywh(rows, input_size, original_size, use_objectness=True)
        without_obj = decode_xywh(rows, input_size, original_size, use_objectness=False)

        # Different exports include objectness differently. Use the richer result set.
        detections = with_obj if len(with_obj) >= len(without_obj) else without_obj
